<?php $__env->startSection('title', 'User Dashboard'); ?>

<?php $__env->startSection('content'); ?>

    <div class="max-w-6xl mx-auto p-8">
        <h2 class="text-xl text-gray-400">
            Welcome <?php echo e(Auth::user()->name); ?> 👋
        </h2>
        <!-- HEADER -->
        <div class="flex justify-between items-center mb-8">

            <h1 class="text-3xl font-bold text-cyan-400">
                User Dashboard
            </h1>

            <a href="/addslang" class="bg-cyan-500 px-4 py-2 rounded-lg hover:bg-cyan-600 transition">
                + Add Slang
            </a>

        </div>

        <!-- USER STATS -->
        <div class="grid md:grid-cols-3 gap-6 mb-10">

            <div class="bg-white/5 p-6 rounded-xl text-center">
                <p class="text-gray-400 text-sm">My Submissions</p>
                <p class="text-3xl font-bold mt-2">
                    <?php echo e($mySlangs); ?>

                </p>
            </div>

            <div class="bg-white/5 p-6 rounded-xl text-center">
                <p class="text-gray-400 text-sm">Approved</p>
                <p class="text-3xl font-bold text-green-400 mt-2">
                    <?php echo e($approvedSlangs); ?>

                </p>
            </div>

            <div class="bg-white/5 p-6 rounded-xl text-center">
                <p class="text-gray-400 text-sm">Pending</p>
                <p class="text-3xl font-bold text-yellow-400 mt-2">
                    <?php echo e($pendingSlangs); ?>

                </p>
            </div>

        </div>

        <!-- MY SLANGS -->
        <div class="bg-white/5 p-6 rounded-xl">

            <h2 class="text-xl font-semibold mb-6">
                My Slang Submissions
            </h2>

            <div class="space-y-4">

                <?php $__empty_1 = true; $__currentLoopData = $slangs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $slang): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="flex justify-between items-center border-b border-white/10 pb-4">

                        <div>

                            <p class="font-semibold text-lg">
                                <?php echo e($slang->word); ?>

                            </p>

                            <p class="text-gray-400 text-sm">
                                <?php echo e($slang->meaning); ?>

                            </p>

                            <span
                                class="text-xs
                    <?php echo e($slang->status == 'approved' ? 'text-green-400' : 'text-yellow-400'); ?>">

                                <?php echo e(ucfirst($slang->status)); ?>


                            </span>

                        </div>

                        <!-- ACTION BUTTONS -->
                        <div class="flex gap-3">



                                <?php if($slang->status == 'pending'): ?>
                                    <!-- Edit -->
                                    <a href="<?php echo e(route('slang.edit', $slang->id)); ?>"
                                        class="bg-blue-500 px-3 py-1 rounded text-sm hover:bg-blue-600">
                                        Edit
                                    </a>

                                    <!-- Delete -->
                                    <form action="<?php echo e(route('slang.destroy', $slang->id)); ?>" method="POST">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('DELETE'); ?>

                                        <button class="bg-red-500 px-3 py-1 rounded text-sm hover:bg-red-600">
                                            Delete
                                        </button>
                                    </form>
                                <?php else: ?>
                                    <span class="text-green-400 text-sm font-semibold">
                                        Approved ✔
                                    </span>
                                <?php endif; ?>


                        </div>

                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>

                        <p class="text-gray-400">
                            You haven't submitted any slang yet.
                        </p>
                <?php endif; ?>

            </div>

            <!-- PAGINATION -->
            <div class="mt-6">
                <?php echo e($slangs->links()); ?>

            </div>

        </div>

    </div>

<?php $__env->stopSection(); ?>

<?php echo $__env->make('layout.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\MCA2025\project\slang-app\resources\views/dashboard/user.blade.php ENDPATH**/ ?>